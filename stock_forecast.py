...
def read_stock_csv(
    ...
    # Optional: fill missing business days
...
def interpret_seasonal_period(user_period: int) -> Optional[int]:
    # Return None to indicate non-seasonal; otherwise an int > 1
...
def main():
    parser = argparse.ArgumentParser(
        description="Forecast stock Close prices (SARIMAX) and generate diagnostic plots."
    )
    parser.add_argument("--csv", type=str, required=True, help="Path to Stock CSV (e.g., ./Stock.csv)")
    parser.add_argument("--date-col", type=str, default="Date", help="Date column name (default: Date)")
    ...
    parser.add_argument("--seasonal-period", type=int, default=5,
                        help="Seasonal period (>1). Use 0 or 1 for non-seasonal (default: 5).")
    parser.add_argument("--fill-business-days", action="store_true",
                        help="Fill missing business days via forward fill (off by default).")
    parser.add_argument("--output-dir", type=str, default="./figures", help="Output directory for plots/CSV")
